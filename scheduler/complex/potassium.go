...
func (m *Potassium) SelectStorageNodes(scheduleInfos []resourcetypes.ScheduleInfo, storage int64) ([]resourcetypes.ScheduleInfo, int, error) {
	...
	for i := range scheduleInfos {
		...
		total += updateNodeInfoCapacity(&scheduleInfos[i], storCap)
	}
...
func (m *Potassium) SelectMemoryNodes(scheduleInfos []resourcetypes.ScheduleInfo, quota float64, memory int64) ([]resourcetypes.ScheduleInfo, int, error) {
	...
	nodesInfoLength := len(scheduleInfos)

	// 筛选出能满足 CPU 需求的
	sort.Slice(scheduleInfos, func(i, j int) bool { return len(scheduleInfos[i].CPUMap) < len(scheduleInfos[j].CPUMap) })
	p := sort.Search(nodesInfoLength, func(i int) bool {
		return float64(len(scheduleInfos[i].CPUMap)) >= quota
	})
	...
	p = sort.Search(nodesInfoLength, func(i int) bool { return scheduleInfos[i].MemCap >= memory })
	if p == nodesInfoLength {
	...
	volTotal := 0
	for i, nodeInfo := range scheduleInfos {
...
func (m *Potassium) SelectVolumeNodes(scheduleInfos []resourcetypes.ScheduleInfo, vbs types.VolumeBindings) ([]resourcetypes.ScheduleInfo, map[string][]types.VolumePlan, int, error) {
	...
	for idx, nodeInfo := range scheduleInfos {
		if len(nodeInfo.VolumeMap) == 0 {
			volTotal += updateNodeInfoCapacity(&scheduleInfos[idx], 0)
			continue
		}
		...
		volTotal += updateNodeInfoCapacity(&scheduleInfos[idx], utils.Min(capNorm, capMono))
